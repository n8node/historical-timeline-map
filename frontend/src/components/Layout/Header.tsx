...
const Header: React.FC<HeaderProps> = ({ personCount, lightMap = false }) => {
  ...
  const panel = lightMap ? 'glass-panel-dark' : 'glass-panel';
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-[1000] pointer-events-none">
      <div className="flex items-center justify-between p-4">
        <Link
          to="/"
          className={`pointer-events-auto ${panel} px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.12] transition-colors group`}
        >
        ...
        <div className="pointer-events-auto flex items-center gap-3">
          {!isAdmin && personCount > 0 && (
            <div className={`${panel} px-3 py-2 text-sm text-white/60`}>
